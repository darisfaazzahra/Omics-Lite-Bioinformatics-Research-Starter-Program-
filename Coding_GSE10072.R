...
#Ambil daftar gen signifikan (DEG)
deg <- topTableResults[topTableResults$adj.P.Val < 0.05 & abs(topTableResults$logFC) > 1, ]
...
write.csv(as.data.frame(ego), "GO_Enrichment_GSE10072.csv", row.names = FALSE)
write.csv(as.data.frame(ekegg), "KEGG_Enrichment_GSE10072.csv", row.names = FALSE)

message("Enrichment GO dan KEGG selesai. File GO & KEGG telah disimpan.")
